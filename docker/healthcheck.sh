...
# -k: under rmw_zenoh the ros2 cli can sit on SIGTERM forever, and a plain `timeout` then never returns —
# a check that runs before the version is published would hang instead of failing.
got=$(timeout -k 2 "${HEALTH_STEP_TIMEOUT:-4}" ros2 topic echo --once \
...
timeout -k 2 "${HEALTH_STEP_TIMEOUT:-4}" ros2 action list 2>/dev/null | grep -qx "$action" \
