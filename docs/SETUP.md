...
sudo ./76-robot-zero-watchdog-install.sh install
./70-dcgm.sh install
...
| Robot zero's watchdog | `76-robot-zero-watchdog-install.sh install`, `status`, `remove`: a root timer, every minute: kills pose queries that outlived their timeout, and restarts the world when the renderer has heard `r00` at under 10 state messages a second for four checks in a row (about two and a half minutes of "Waiting for sim"). Everything it does is in `journalctl -u robot-zero-watchdog` | robot zero installed |
