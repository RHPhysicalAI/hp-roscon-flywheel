...
4. aarch64 availability (Phase 3) — **retired 2026-09-19 except RHTAS (D147):** on the 4.22 arm64 hub, OpenShift GitOps 1.21, Pipelines 1.24, the Cluster Observability Operator, Tempo, RHOAI 3.5 (dashboard behind the Gateway API included), MinIO, Kafka, Perses and RHEM 1.3 with its product UI all install and run; all eight Argo Applications are Synced/Healthy. Two amd64-only images had to be swapped (the RHEM UI and the chart's CLI image — D146/D147). **RHTAS still has no arm64 server images**: Rekor + Trillian are being rebuilt from the midstream source (`tools/host/fury/rhtas-arm64/`, one-day time box, upstream `rekor` chart as the fallback).
...
6. Empty/retargeted rollout batches (Phase 3.4).
