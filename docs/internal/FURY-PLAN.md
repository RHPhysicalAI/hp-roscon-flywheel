...
| L4 | Promotion reset: put the hub and the device back to "teacher serving, no v2" so the beat can be rehearsed and re-shown | — | **not built** — next | 5, 10 | script: Fleet pin and consumer env back to the teacher (a revert of the promotion merge), the registry version, the catalog item, the consumer's count; keep the run's `eval/<run_id>/` objects; every re-run now leaves its images tagged (D158) |
