...
4. aarch64 availability (Phase 3) — **partly retired 2026-09-18:** RHOAI/DSP, Tempo, MinIO, Kafka, Perses, and flightctl 1.3.0 API/agent are verified arm64; **RHTAS is not (no arm64 images at all) and the RHEM UI is not** — see decisions 10/10b. Still unchecked: OpenShift Pipelines and COO operator bundles on the aarch64 catalog.
5. KVM/UEFI aarch64 guest on Grace (Phase 2) — unproven anywhere in the project.
