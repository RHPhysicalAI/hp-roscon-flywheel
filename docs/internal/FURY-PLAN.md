...
|---|---|---|---|---|---|
| L1 | Staged teacher → v2 promotion: both checkpoints, the 161-episode dataset and the 360-seed paired evaluation put where the runner reuses them, so a governed run takes minutes | `tools/host/fury/53-stage-promotion.sh`; inputs under `/data/models/import-dev` | **done 2026-09-20** (D158) | 5 | — |
| L2 | Teacher modelcar packaged and signed on the Fury, so the Fleet can start at the teacher and the rollout shown is teacher → v2 | — | **done 2026-09-20** (D158): the existing teacher modelcar signed by this hub (`tools/hub/cosign-image.sh`), checked by `54-teacher-modelcar-check.sh`, Fleet at the teacher in `6f69dd2` | 5 | — |
| L3 | The act 1 run itself: `INCUMBENT=upstream-act-teacher COLLECTOR=upstream-act-teacher INCUMBENT_CHECKPOINT=s3://episodes-data/checkpoints/upstream-act-teacher/pretrained_model.tar.gz tools/hub/start-promotion-run.sh act-v2-ft160 0.25 360` | `tools/hub/start-promotion-run.sh` | **done 2026-09-20** (D158): run `9fb233e8`, PASS, PR #7, merge → serving in 1 min 38 s | 5 | — |
| L4 | Promotion reset: put the hub and the device back to "before the last promotion" so the beat can be rehearsed and re-shown | `tools/hub/reset-promotion.sh` | **built 2026-09-20, not yet exercised** (dry run up to the confirmation only): reverts the newest promotion merge in a scratch worktree, pushes, waits for the device | 5, 10 | run it once, then `start-promotion-run.sh act-v2-ft160 0.25 360` again - that re-run is also the first exercise of the per-run image tags (D158) |
| L5 | Collection scene: the Fury's coordinator unit ran the image-default scene (fixed cubes, 25 s episodes), not the project's collection scene (randomized medium cube, 60 s) | `tools/host/fury/flywheel/act-coordinator.container` | **fixed in the unit 2026-09-20** (`818afb5`; no `RESET_ARM` in the loop - it breaks it), **not yet installed on the host** | 5 | operator: `./14-flywheel-services.sh`, then start the recorder; check the live page's rate for the serving model comes up towards the evaluated one |
...
| L9 | Assistant as a host service that `fury-mode tenants` starts on slice `0:0` (D156) | `tools/host/fury/63-assistant-install.sh`, `flywheel/llm-assistant.container`, `flywheel/llm-cache.volume`, `fury-mode.sh` | **done 2026-09-20** (D160): up in under three minutes after the switch; 0.132 s to first token, 245.7 tokens/s decode, tool call well-formed | 6 | how the booth reaches it and talks to it (it listens on loopback); the isolation beat; shred the pull secret |
...
12. *(2026-09-19)* If 11 passes: MuJoCo-Warp renderer speed and look on the real scene in a 1g slice, and how far the current policy is from those pixels. **Batch rate retired 2026-09-20 (D163 addendum):** about 228 camera pairs/s per 1g.31gb slice regardless of batch size - 7 robots at 30 fps, about 20 at 480x480 and 15 fps; geometry matches Gazebo to a tenth of a pixel. How the policy does on those pixels no longer matters (D163). **Speed retired 2026-09-19:** 152 frame pairs/s with shadows on a 1g.31gb slice for the real 322k-face scene (D143 addendum); the look differs from Gazebo's, so the policy gap is still open.
